...
function SeriesPageInner() {
  ...
  const popular = useQuery({
    queryKey: ["vod", "popular_tv"],
    queryFn: () => api.vodBrowse("popular_tv"),
  });
  const topRated = useQuery({
    queryKey: ["vod", "top_rated_tv"],
    queryFn: () => api.vodBrowse("top_rated_tv"),
  });
  const onAir = useQuery({
    queryKey: ["vod", "on_the_air_tv"],
    queryFn: () => api.vodBrowse("on_the_air_tv"),
  });
  ...
  return (
    <div className="@container flex h-full flex-col gap-6 overflow-y-auto p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold @lg:text-3xl">Series</h1>
          <p className="text-sm text-white/60">
            Discover TV shows you'll want to binge
          </p>
        </div>
      ...
      {debounced.trim().length > 1 ? (
        ...
        <>
          <PosterRow
            ...
            items={trending.data?.filter((i) => i.kind === "tv")}
