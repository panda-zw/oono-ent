...
//
// `base: "./"` matters: WKWebView serves the bundle through the
// `oono-app://` scheme (see ios/OonoEnt/AppBundleHandler.swift), so all
// asset URLs must be relative.
//
// A TMDb API key can be baked in via `.env.local` (VITE_TMDB_KEY=…) so the
// iPad app works without re-entering the key in Settings. The key is
// inlined via Vite's `define`, never imported by the desktop build.
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      __OONO_DEFAULT_TMDB_KEY__: JSON.stringify(env.VITE_TMDB_KEY ?? ""),
    },
    build: {
